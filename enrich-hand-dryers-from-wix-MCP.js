...
// ============================================
// COMPLETE WIX ENRICHMENT SCRIPT (MCP VERSION)
// ============================================
...
console.log('🛡️  COMPLETE WIX ENRICHMENT SCRIPT (MCP VERSION)');
console.log('='.repeat(70));
console.log('');
...
function extractWixDescriptions(wixProduct) {
    ...
    if (wixProduct.productPageUrl) {
        // Product page URL can be used to fetch additional content
...
async function processWixProducts(wixProducts) {
    ...
    for (const wixProduct of wixProducts) {
        ...
        console.log(`   📝 Description: ${wixDescriptions.descriptionFull ? 'Yes' : 'No'}`);
...
async function enrichHandDryers(wixProducts) {
    if (!wixProducts || wixProducts.length === 0) {
        console.log('\n⚠️  No Wix products provided.');
        console.log('   Use MCP tools to fetch hand dryers from Wix first.');
        return;
    }

    return await processWixProducts(wixProducts);
}
...
if (require.main === module) {
    console.log('\n📋 INSTRUCTIONS:');
    console.log('='.repeat(70));
    console.log('\nThis script enriches hand dryers with Wix data.');
    console.log('\nTo use with MCP:');

...
console.log('🔍 Checking products-with-grants.json for government info...');

try {
    const data = JSON.parse(fs.readFileSync('products-with-grants.json', 'utf8'));
    ...
    // Check refrigeration products
    const refrigerationProducts = data.products.filter(p => 
    ...
    console.log(`\n🧊 Refrigeration products: ${refrigerationProducts.length}`);
    ...
    if (hasGrants && hasCollectionAgencies) {
        ...
        console.log(`✅ Contains ${refrigerationProducts.length} refrigeration products`);
    } else {

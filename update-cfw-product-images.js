...
console.log('\n🔄 UPDATING CFW501 AND CFW701 PRODUCT IMAGES');
console.log('='.repeat(70));
console.log('');

// Database path
const dbPath = path.join(__dirname, 'FULL-DATABASE-5554.json');
const cfw501ImagePath = 'Product Placement/CFW501 Series Frequency Inverters - HVAC Drive.jpg';
const cfw701ImagePath = 'Product Placement/cfw701.jpg';
...
const backupPath = path.join(__dirname, `FULL-DATABASE-5554-CFW-BACKUP-${timestamp}.json`);
fs.copyFileSync(dbPath, backupPath);
console.log(`💾 Backup created: FULL-DATABASE-5554-CFW-BACKUP-${timestamp}.json\n`);
...
data.products.forEach(product => {
    ...
    if (name.includes('cfw501') || name.includes('cfw 501')) {
        ...
        if (product.imageUrl !== cfw501ImagePath) {
            ...
            product.imageSource = 'cfw501-specific';
...
console.log(`✅ Updated ${cfw501Updated} CFW501 products\n`);
...
data.products.forEach(product => {
    ...
    if (name.includes('cfw701') || name.includes('cfw 701')) {
        ...
        if (product.imageUrl !== cfw701ImagePath) {
            ...
            product.imageSource = 'cfw701-specific';
            product.imageUpdated = new Date().toISOString();
            cfw701Updated++;
            console.log(`   ✅ ${product.name}`);
...
console.log(`✅ Updated ${cfw701Updated} CFW701 products\n`);

// Save database
if (cfw501Updated > 0 || cfw701Updated > 0) {
    fs.writeFileSync(dbPath, JSON.stringify(data, null, 2));
    console.log(`✅ Database saved with CFW product images!`);
    ...
    console.log('\n✅ All CFW products now have specific images!\n');
} else {

...
console.log('\n🔄 UPDATING CFW501 AND CFW701 TO EDITED IMAGES');
console.log('='.repeat(70));
console.log('');

// Database path
const dbPath = path.join(__dirname, 'FULL-DATABASE-5554.json');
const cfw501ImagePath = 'Product Placement/CFW501 Series Frequency Inverters - HVAC Drive_edited.jpeg';
const cfw701ImagePath = 'Product Placement/cfw701_edited.jpg';
...
const backupPath = path.join(__dirname, `FULL-DATABASE-5554-CFW-EDITED-BACKUP-${timestamp}.json`);
fs.copyFileSync(dbPath, backupPath);
console.log(`💾 Backup created: FULL-DATABASE-5554-CFW-EDITED-BACKUP-${timestamp}.json\n`);
...
data.products.forEach(product => {
    ...
    if (name.includes('cfw501') || name.includes('cfw 501')) {
        ...
        if (product.imageUrl !== cfw501ImagePath) {
            ...
            product.imageSource = 'cfw501-edited';
...
console.log(`✅ Updated ${cfw501Updated} CFW501 products to edited image\n`);
...
data.products.forEach(product => {
    ...
    if (name.includes('cfw701') || name.includes('cfw 701')) {
        ...
        if (product.imageUrl !== cfw701ImagePath) {
            ...
            product.imageSource = 'cfw701-edited';
            product.imageUpdated = new Date().toISOString();
            cfw701Updated++;
            console.log(`   ✅ ${product.name}`);
...
console.log(`✅ Updated ${cfw701Updated} CFW701 products to edited image\n`);

// Save database
if (cfw501Updated > 0 || cfw701Updated > 0) {
    fs.writeFileSync(dbPath, JSON.stringify(data, null, 2));
    console.log(`✅ Database saved with CFW edited images!`);
    ...
    console.log('\n✅ All CFW products now have edited images!\n');
} else {

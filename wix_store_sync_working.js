...
console.log('🛒 WIX STORE SYNC SYSTEM (WORKING)\n');
...
class WixStoreSync {
    constructor() {
        this.syncConfig = {
            // Select which categories to sync to store (using actual category names)
            enabledCategories: [
                'Hand Dryers',
                'Heat Pumps', 
                'Lighting',
        ...
        return new Promise((resolve, reject) => {
            const sql = `
                SELECT 
                    ...
                    source
                FROM products 
                WHERE category IN (${this.syncConfig.enabledCategories.map(() => '?').join(',')})
                AND energy_rating >= ?
                AND source NOT IN (${this.syncConfig.excludeSources.map(() => '?').join(',')})
                ORDER BY category, energy_rating DESC, running_cost_per_year ASC
            `;
    ...
    groupAndLimitProducts(products) {
        ...
        products.forEach(product => {
            ...
            if (grouped[product.category].length < this.syncConfig.maxProductsPerCategory) {
        ...
        Object.entries(products).forEach(([category, categoryProducts]) => {
            categoryProducts.forEach(product => {
                wixProducts.push({
                    ...
                    sku: product.id, // Use the unique ID as SKU
    ...
    generateProductDescription(product) {
        return `Energy-efficient ${product.category} by ${product.brand}. 
...
async function testStoreSync() {
    ...
    try {
        ...
        console.log('   - Each product has a unique ID (etl_9_12345)');
        console.log('   - Use this ID as the SKU to prevent duplicates');
        console.log('   - Check existing products before adding new ones');

...
- **`POST /api/assistant/ask`**, **`GET /api/assistant/context`**, **`GET /api/assistant/status`** — uses `DashboardLiveService`, optional shared server LLM (`ASSISTANT_PROVIDER` / `ASSISTANT_API_KEY` / `ASSISTANT_MODEL` via **`greenways-agent-llm.js`** — same stack as Transition Agents), else heuristic answer + **confidence / assumptions / ROI** block. Ask returns **`llmUsed`** / **`mode`**.
- **Wok Assist system prompt (Jul 2026):** server LLM uses **`services/wok-assist-system-prompt.js`** (brand + five intelligence domains + site energy/deals/schemes JSON). **`Chef 3 W2W .html`** defaults to this server path; Options can optionally override with a browser API key.

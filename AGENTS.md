...
| **Site energy reading** | `HTMLS GWM GWB/site-energy-reading.html` · `/greenways/site-energy-reading` (+ `-embed`) — postcode → site utility connections + grid carbon; module `site-energy-reading`; demo: `?theme=light&display=1&embed=1`. Optional UK **EPC heating hint** when `EPC_OPEN_DATA_EMAIL` + `EPC_OPEN_DATA_API_KEY` (or `EPC_API_*`) are set — user applies suggestion, then Save for agents. After lookup, **Utility lanes** cards deep-link by enabled connection (electricity / gas / water). Saved connections (`siteConnections` / `sitePostcode`) flow into agent `/ask` via `greenways-agent-team.js` + `services/site-energy-profile.js` (Vincent / Artemis / Edwardo / Zara / Zyanne cite them in answers). |
...
| **Low-energy equipment savings** | `HTMLs/Low-energy-equipment saving.html` · `/greenways/low-energy-equipment-savings` |
...
| **Agents admin (staff)** | `agents-admin.html` · `GET /api/agents-admin/overview` · `GET /api/agents-admin/graph` · `POST /api/agents-admin/content-modules` · `data/greenways-agent-admin-registry.json` — overview table + links to `schemes-admin.html` |
| **Agents network map (staff)** | `agents-admin-map.html` · `/api/agents-admin/graph` — spider diagram (portraits, JSON data files, **portal tools** from `greenways-content-modules.json`, handoffs, **19 live referral welcomes**); **click-to-pin** selection; **Add to toolkit** form writes content modules |
...
| **Organisation Desk (plan board)** | `HTMLS GWM GWB/greenways-organisation-desk.html` · `/greenways/organisation-desk` · PRD `tasks/prd-organisation-desk.json` — board + calendar; Add to desk; member sync `GET/PUT /api/members/organisation-desk` · **⚠️ banner MutationObserver gotcha** (Learnings Sep 2026) |
...
- **Organisation Desk member sync**: Logged-in members use `GET/PUT /api/members/organisation-desk` (Mongo `organisationDesk` + SQLite `organisation_desk`). Client merges with `localStorage` `greenways-organisation-desk-v1` and never wipes local cards on API failure. Smoke: open desk signed-in → Sync account → reload still shows cards.
...
- **🤝 Agent referral welcomes (Jun 2026):** `services/greenways-agent-handoff.js` — **19 live pairs** (13 specialist→specialist + any specialist→Edwardo). First `/ask` with `profile.handoff` returns `agent_referral_welcome` with grounded samples. Staff **network map:** `agents-admin-map.html` · API `GET /api/agents-admin/graph` · **`POST /api/agents-admin/content-modules`** (register portal tools). Smoke: `npm run smoke:agents-ask`.
- **🗺️ Agents admin map — portal toolkit (Jun 2026):** `agents-admin-map.html` + `services/agents-admin-service.js` — middle ring = **`data/greenways-content-modules.json`** (agent ↔ tool edges); sidebar **Add to toolkit** appends modules; **click-to-pin** keeps detail panel while scrolling. Sidebar quick links still need **`npm run sync:agent-sidebar`** when adding links. Site knowledge cards (`data/greenways-site-knowledge/`) remain Phase 2 backlog.
- **📖 Agents portal pages (Jun 2026):** **`greenways-agents-story.html`** (`/greenways/agents-story`) + **`greenways-agents-highlights.html`** (`/greenways/agents-highlights`) + Orchestra hub — shared nav; story intro positions business development + government + hospitality context.
...
- **🔄 Agents data refresh playbook (Jun 2026):** Staff pipeline in **`Skills/agents-data-refresh-playbook.md`** · manifest **`data/agents-data-pipeline.json`** · `npm run refresh:agents-data` (checklist) · `npm run validate:agent-data` (smokes). **Guide Orchestrator** intent `route_data_refresh`. **Agents admin** registry links playbook; Edwardo verify stays read-only.
- **🚀 Launch mode — agents (Jun 2026):** Wix launch prioritises experience over LLM caps. Policy in **`Skills/greenways-agents-roadmap.md`** § Launch mode. **Track A:** `finishKnowledgeAskResponse()` on all seven `/ask` routes; **`npm run smoke:agents-ask`**. LLM polish via `GREENWAYS_AGENT_POLISH_AGENTS` (default `finance` only). **Track B (Vincent):** `buildAgentHandoff` + `FINANCE_HANDOFF_RULES`; conversational `blocks[]` on `energy_prices`, `bnpl`, `green_loans`, `price_upgrade_case`, `etl_products`. **Vincent module tablets (Phase 1):** embeddable site pages open in **`GreenwaysAgentContentModule`** (`theme: finance`); copy from **`data/greenways-content-modules.json`** — **`description`** (what it is) + **`usageHint`** (how to use) in modal header and chat tablets. **Andrieus module tablets (Jun 2026):** same shell with `theme: grants`; registry ids **`schemes-portal-restaurant`**, **`schemes-portal-eu`**; portal intents return `type: "module"` blocks; sidebar quick links use `moduleOpen`. **Artemis module tablets (Jun 2026):** `theme: equipment`; deep dive, renovations, insulation, renovation plans, savings projection, ETL finder — registry + `equipment-agent-knowledge.js` module blocks. **Cheryce module tablets (Jun 2026):** `theme: media`; news/references/monitoring/water/deals/ticker sidebar `moduleOpen`; map keeps `mapOpen`; `media-agent-knowledge.js` portal intents → `module` blocks with Cheryce theme. **Zara module tablets (Jun 2026):** `theme: deals` (cyan/orange); registry id **`deals-full-page`** plus **`european-energy`**, **`deals-ticker`**, **`water-saving-finder`**; `deals-agent-knowledge.js` portal intents → `module` blocks; sidebar quick links `moduleOpen`. **Zyanne module tablets (Jun 2026):** `theme: products` (blue/teal); water finder, product finder, equipment deep dive, eco planner — `sustainable-products-agent-knowledge.js` `linkOrModuleBlocks`; sidebar `moduleOpen`; full `ContentModule` intercept. **Edwardo module tablets (Jun 2026):** `theme: systems` (amber/gold); monitoring guide, sensor dashboard, buildings dashboard — `systems-agent-module-blocks.js`; sidebar **Ops verify** block unchanged; quick links `moduleOpen`; chat uses TurnUi split + module tablets like other agents. **Chat tablets standard (Jun 2026):** Zara-style split (left summary + right **module tablets** + formatted awareness panel) is the rollout target for **all agents** — see **`Skills/greenways-chat-interface-skill.md`** § Conversational answer pattern; shared formatting in **`greenways-agent-turn-ui.js`** / **`.css`**.
- **⚡ Platform power roadmap (Jul 2026):** Nine waves (member context → upgrade plans → freshness → chat actions → copy voice parity → proactive nudges → analytics → **agent voice** → **agent email**). Ralph PRD **`tasks/prd-agents-power-platform.json`** — say *“Start Ralph loop for agents power platform”*. **Wave 8:** `greenways-agent-voice.js` + `spokenSummary` → premium TTS (SkillBoss/ElevenLabs optional). **Wave 9:** agent mailboxes + **Email me this** (SkillBoss `aws/send-emails` optional).

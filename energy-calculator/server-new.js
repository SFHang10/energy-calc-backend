...
console.log('✅ /api/wix route mounted');
console.log('All routes mounted successfully');

// Test ETL route directly
app.get('/api/etl/test', (req, res) => {
    res.json({ message: 'ETL test route working!', timestamp: new Date().toISOString() });
});
console.log('✅ /api/etl/test test route added');
...
app.get('/api/calculator-wix/products', async (req, res) => {
  try {
    ...
    const etlResponse = await fetch(`${process.env.ETL_BASE_URL || 'https://api.etl.energysecurity.gov.uk'}/products`, {
      headers: {
    ...
    if (etlResponse.ok) {
      ...
      const products = etlData.products.map(product => ({
        id: product.id,
        name: product.name,
        power: product.power_usage || product.power_consumption || 0,
        category: product.category,
        brand: product.manufacturer || 'Unknown',
        runningCostPerYear: calculateRunningCost(product.power_usage || 0),
        energyRating: product.energy_rating || 'Unknown',
        efficiency: product.energy_rating === 'A+++' || product.energy_rating === 'A++' ? 'high' : 'medium'
      }));
      res.json(products);
    } else {
...
app.listen(PORT, () => {
  ...
  console.log(`   - POST /api/members/login (Member login)`);

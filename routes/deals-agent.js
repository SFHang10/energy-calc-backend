...
router.get('/samples', async (req, res) => {
  try {
    ...
    console.error('Deals agent samples error:', error.message);
    res.status(500).json({ ok: false, error: 'Failed to load deal samples.' });
  }
});
...
router.post('/ask', async (req, res) => {
  ...
  try {
    ...
    const response = await finishKnowledgeAskResponse('deals', knowledge, question, profile);
    if (response) {
    ...
    const fallback = await buildAgentAskFallback('deals', question, profile);

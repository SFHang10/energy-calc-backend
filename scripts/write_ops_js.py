...
js = r'''let opsLivePulseTimer = null;
...
# sanitize accidental motion tags from editor
js = js.replace('<motion ', '<motion ').replace('</motion>', '</motion>')
js = js.replace('<motion ', '<div ').replace('</motion>', '</div>')
...
function updateDataQualityPanel() {
  ...
  panel.innerHTML = rows.map((r) => {
    ...
    return (
      ...
      '<div class="ops-meter-track"><motion class="ops-meter-fill' + fillCls + '" style="width:' + r.pct + '%"></div></div>' +
      '</div>'
    ).replace('<motion class="ops-meter-fill', '<div class="ops-meter-fill').replace('</div></motion>', '</div>');
  }).join('') + '<motion class="ops-link-row"><button type="button" id="opsOpenSensorDashboard">Open sensor dashboard →</button></div>';
  panel.innerHTML = panel.innerHTML.replace('<motion class="ops-link-row">', '<div class="ops-link-row">');
...
function updateAuditTrail(app) {
  ...
  panel.innerHTML = items.map((item) => {
    ...
    return (
      ...
      '</div>' +
      '<div class="audit-feed-sub">' + item.sub + subSuffix + '</motion>' +
      '</div></div>'
    ).replace('</motion>', '</div>');
  }).join('');
}
...
out = js + rest
out = out.replace('<motion ', '<motion ').replace('</motion>', '</motion>')
# final sanitize
import re
out = re.sub(r'</?motion[^>]*>', lambda m: '<div>' if m.group(0).startswith('<motion') and not m.group(0).startswith('</') else ('</motion>' if False else ''), out)
# simpler
out = out.replace('<motion ', '<div ').replace('</motion>', '</div>')

Path(__file__).parent.joinpath('_ops_js_interactive.js').write_text(out, encoding='utf-8')

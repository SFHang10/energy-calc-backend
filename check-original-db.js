...
console.log('🔍 Checking energy_calculator.db schema and content...\n');
...
db.all("PRAGMA table_info(products)", (err, rows) => {
    ...
    console.log('📋 Schema of energy_calculator.db:');
    rows.forEach(row => {
    ...
    db.get('SELECT COUNT(*) as count FROM products', (err, countRow) => {
        ...
        db.all('SELECT category, COUNT(*) as count FROM products GROUP BY category ORDER BY count DESC', (err, categoryRows) => {
            ...
            db.all('SELECT name, brand, category FROM products LIMIT 10', (err, sampleRows) => {
                if (err) {
                    console.error('Error:', err);
                    return;
                }
